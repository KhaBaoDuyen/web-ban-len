...
export default function QuanLySanPham() {
    ...
    const filteredProducts = products.filter((p) => {
        ...
        const matchesTab = (p.categoryId === activeTab) || (p.category?._id === activeTab);
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const res = await fetch(`/api/products/${selectedSlug}`, { method: "DELETE" });
            if (res.ok) {
    ...
    return (
        <div className="min-h-screen p-4 md:p-8">
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-1">
                    <div>
                        ...
                        <p className="text-slate-500 text-md">
                            Hiện có {filteredProducts.length} sản phẩm (Trang {currentPage}/{totalPages || 1})
                        </p>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between mt-3 mb-4">
                    <div className="relative w-full sm:w-96">
                        <input
                            ...
                            className="w-full px-4 py-2.5 shadow bg-white rounded-xl border border-slate-200 focus:border-indigo-50 focus:ring-4 focus:ring-indigo-50 outline-none transition-all"
                        />
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-4 mb-2 no-scrollbar">
                    <button
                        ...
                        className={`px-4 py-2 rounded-xl whitespace-nowrap text-sm font-medium transition-all border ${activeTab === "all" ? "bg-primary-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-200 hover:bg-slate-50"
                            }`}
                    >
                        Tất cả danh mục
                    </button>
                        ...
                        <table className="w-full min-w-[700px] border-collapse">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {currentProducts.length === 0 ? (
                                    <tr><td colSpan={5} className="text-center py-10 text-slate-400">Không tìm thấy sản phẩm</td></tr>
                                ) : (
                                    currentProducts.map((product, index) => (
                                        <tr key={product.slug} className="hover:bg-slate-50/80 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 text-slate-500 text-md max-w-xs text-justify line-clamp-5 ">{product.description}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-center text-md font-bold text-slate-700">{formatVND(Number(product.price))}</td>
                                            <td className="px-6 py-4 text-center whitespace-nowrap">
                                                <button onClick={() => toggleStatus(product.slug, product.status)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${product.status === 1 ? "bg-green-500" : "bg-slate-300"}`} >
                                                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${product.status === 1 ? "translate-x-5" : "translate-x-1"}`} />
                                                </button>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-md">
                                                <div className="flex items-center justify-end gap-2">
                                                    <Link href={`/quan-ly-san-pham/${product.slug}`} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all"><PencilSquareIcon className="w-5 h-5" /></Link>
                                                    <button onClick={() => { setSelectedSlug(product.slug); setOpenDelete(true); }} disabled={isDeleting} className={`p-2 rounded-lg transition-all ${isDeleting ? "opacity-50 bg-slate-100" : "text-slate-400 hover:text-red-600 hover:bg-red-50"}`}><TrashIcon className="w-5 h-5" /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
                            ...
                            <div className="flex gap-3">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-xs text-slate-400 mt-0.5">
                                        ID: #SP-{1000 + startIndex + index + 1}
                                    </span>                                    <span className="text-sm font-bold text-slate-700 mt-1">{formatVND(Number(product.price))}</span>
                                    <p className="text-xs text-slate-500 mt-1 line-clamp-2">{product.description}</p>
                                </div>
                            </div>
                            <div className="flex flex-col gap-2 ml-2">
                                <Link href={`/quan-ly-san-pham/${product.slug}`} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all"><PencilSquareIcon className="w-5 h-5" /></Link>
                                <button onClick={() => { setSelectedSlug(product.slug); setOpenDelete(true); }} disabled={isDeleting} className={`p-2 rounded-lg transition-all ${isDeleting ? "opacity-50" : "text-slate-400 hover:text-red-600 hover:bg-red-50"}`}><TrashIcon className="w-5 h-5" /></button>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {totalPages > 1 && (
                    <div className="flex items-center justify-center gap-2 mt-8">
                        <button
                            ...
                            className="p-2 rounded-lg border border-slate-200 disabled:opacity-30 bg-white"
                        >
                ...
                {openDelete && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                        <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-xl animate-in fade-in zoom-in">
                            <h2 className="text-lg font-bold text-slate-800">Xác nhận xoá sản phẩm</h2>
                            <p className="text-slate-500 mt-2">Bạn có chắc chắn muốn xoá sản phẩm này không? Hành động này không thể hoàn tác.</p>
                            <div className="flex justify-end gap-3 mt-6">
                                <button onClick={() => setOpenDelete(false)} disabled={isDeleting} className="px-4 py-2 rounded-xl border font-semibold text-slate-600 hover:bg-slate-100">Huỷ</button>
                                <button onClick={handleDelete} disabled={isDeleting} className="px-4 py-2 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700 disabled:opacity-50">
